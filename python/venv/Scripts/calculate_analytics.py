...
def analyze_posts():
    """
    Analyze posts that are new or lack analytics.
    """
    # Find posts that don't have corresponding analytics or were recently added
    recent_time = datetime.utcnow() - timedelta(minutes=5)  # Process posts from last 5 minutes
    posts = posts_collection.find({
        "$or": [
            {"_id": {"$nin": analytics_collection.distinct("postId")}},  # Posts without analytics
            {"createdAt": {"$gte": recent_time}}  # Recently added posts
        ]
    })
    ...
    # If no posts, exit
    if not posts_list:
        print("No new or unanalyzed posts found.")
        return
